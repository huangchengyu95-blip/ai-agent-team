...
**输出格式（必须是JSON）：**
{
  "knowledge_update": "认知沉淀文档的【完整内容】（Markdown格式）。必须包含原有全部内容+本次新增洞察，整合后重新排版，结构清晰，可直接替换整个文档。不能只写增量，必须是完整版。",
...
def _get_recent_ideas(n: int = 10) -> list:
    """从 status.json 读取最近 n 条产品创意标题，用于创意去重"""
    import os
    status_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "status.json")
    try:
        with open(status_path, "r", encoding="utf-8") as f:
            status = json.load(f)
        # 从 activity_log 里找评审员完成评审的记录，提取标题
        ideas = []
        for entry in status.get("activity_log", []):
            if entry.get("agent") == "product_reviewer" and "完成评审《" in entry.get("message", ""):
                msg = entry["message"]
                start = msg.find("《") + 1
                end = msg.find("》")
                if start > 0 and end > start:
                    ideas.append(msg[start:end])
        # 去重并取最近 n 条
        seen = set()
        unique_ideas = []
        for idea in ideas:
...
def run(feishu_client: FeishuClient = None, llm_client: LLMClient = None,
    ...
    try:
        ...
        # 读取历史产品创意列表，用于去重
        recent_ideas = _get_recent_ideas(n=10)
...
**第三步：整合重写认知框架**
基于新信息，对认知沉淀文档进行整合重写：
- 将新洞察融入已有内容的对应章节，而不是简单追加在末尾
- 修正或补充已有章节中不够准确或不够完整的内容
- 保证文档结构完整（六个章节都要有内容），语句流畅，消除重复
- 输出的是可以直接替换整个文档的**完整版本**，不是只写新增的增量部分
